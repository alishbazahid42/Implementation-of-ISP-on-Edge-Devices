"""
run_nm_benchmark.py  —  CUDA C inference on 2417 images using New Module weights
"""
import os, sys, json, subprocess, time, tempfile, math
import numpy as np
from PIL import Image
from skimage.metrics import structural_similarity as ssim_fn

EXE      = r"C:\Users\aujla\Desktop\archive\cuda_isp\syenet_isp.exe"
WEIGHTS  = r"C:\Users\aujla\Desktop\archive\new_module\weights_nm.bin"
...
OUT_JSON = r"C:\Users\aujla\Desktop\archive\nm_benchmark_results.json"
...
def gt_to_float32(png_path):
    return np.array(Image.open(png_path).convert('RGB')).astype(np.float32)/255.0

fnames = sorted(f for f in os.listdir(RAW_DIR) if f.endswith('.png'))
total  = len(fnames)
print(f"Found {total} images. Running CUDA C inference (New Module weights)...")

results = []
t_start = time.time()
tmp_in  = os.path.join(tempfile.gettempdir(), "syenet_nm_input.bin")
tmp_out = os.path.join(tempfile.gettempdir(), "syenet_nm_output.bin")
...
for i, fname in enumerate(fnames):
    ...
    raw_to_float32(raw_path).tofile(tmp_in)

    t0 = time.perf_counter()
    proc = subprocess.run([EXE, WEIGHTS, tmp_in, tmp_out, "1"],
    ...
    out_hwc = np.fromfile(tmp_out, dtype=np.float32).reshape(3,256,256).transpose(1,2,0).clip(0,1)
    gt_hwc  = gt_to_float32(gt_path)

    img_psnr = psnr(out_hwc, gt_hwc)
    img_ssim = float(ssim_fn(out_hwc, gt_hwc, data_range=1.0, channel_axis=2, win_size=7))
    ...
    if (i+1) % 100 == 0 or i == 0:
        ...
        print(f"  [{i+1:4d}/{total}]  PSNR={avg_p:.3f}  SSIM={avg_s:.4f}"
...
summary = {
    ...
    "model":          "New Module (SYEISPNet channels=12, rep_scale=4)",
    "weights_file":   "weights_nm.bin  (22.1 KB)",
    "psnr": {
